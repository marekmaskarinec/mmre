...
void _log(enum logLevel level, char *msg, ...);
#ifdef NDEBUG
#define log _log
#else
#define log(lvl, ...) \
...
#define assertf(exp, ...) \
	do { \
		if (!exp) log(LOG_FAT, __VA_ARGS__); \
	} while (0)
...
void send_email_SMTP(
	...
	const char *pwd,
	const char *msg);
int parse_RSS(const char *url, const char *text, void (* callback)(void *user, struct post *), void *user);
void entry_free(struct entry *entry);
void user_free(struct user *user);
size_t load_hashes(uint64_t **hashes, struct user *user, const char *url);
