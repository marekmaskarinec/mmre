...
get_url(const char *url)
{
	...
	struct wbuf b = (struct wbuf){
		.str = calloc(32, sizeof(char)),
		.cap = 32
	};
	
	curl_easy_setopt(curl, CURLOPT_URL, url);
	curl_easy_setopt(curl, CURLOPT_USERAGENT, "mmre v0.1 - RSS feed to email daemon https://mrms.cz/mmre");
	curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, getUrl__writeFunction);
	curl_easy_setopt(curl, CURLOPT_WRITEDATA, &b);
	
	assertg(fail, !curl_easy_perform(curl), "Could not perform request to %s", url);
...
static size_t
sendEmailSMTP__readFunction(char *ptr, size_t size, size_t nmemb, struct rbuf *b)
{
	...
	const char *msg)
{
	...
	const char *weekdays[] = { "Sun", "Tue", "Wed", "Thu", "Fri", "Sat", "Mon" };
	const char *months[] = {
	...
	assertg(cleanup, !curl_easy_perform(curl), "Could not send email to %s.", to);
