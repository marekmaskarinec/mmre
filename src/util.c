...
_log(enum logLevel level, char *msg, ...) {
	...
	fprintf(
	    log_file,
	    "%04d-%02d-%02dT%02d:%02d:%02d: [%s]: ", tm->tm_year + 1900,
...
load_hashes(uint64_t **hashes, struct user *user, const char *url) {
	...
	snprintf(
	    buf, BUFSIZ - 1, "/var/lib/mmre/%s/%lx", user->email, djb2(url)
	);
	FILE *f = fopen(buf, "rb");
	if (f != NULL) {
	...
	f = fopen(buf, "wb");
	assertf(f, "Could not open file %s.", buf);
	log(LOG_DBG, "%s was not found, downloading feed from %s.", buf, url);
	char *data = get_url(url);
	assertg(
	    fail, !parse_RSS(url, data, write_hashes_callback, f),
	    "Could not parse RSS"
	);
...
save_hash(struct entry *entry, uint64_t hash) {
	...
	entry->hashes =
	    realloc(entry->hashes, ++entry->nhash * sizeof(uint64_t));
	...
	char buf[BUFSIZ] = {0};
	snprintf(
	    buf, BUFSIZ - 1, "/var/lib/mmre/%s/%lx", entry->owner->email,
	    djb2(entry->url)
	);
	FILE *f = fopen(buf, "wb");
